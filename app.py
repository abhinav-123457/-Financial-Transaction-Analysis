...
def calculate_balances(transactions):
    ...
    target_date = last_date.replace(hour=16, minute=43, second=0, microsecond=0)  # 04:43 PM IST, August 15, 2025
...
def generate_excel(overdue_with_interest, pending_credits, opening_balance, closing_balance, total_credits, total_debits, total_principal, total_interest, gst, total_amount_due, target_date):
    ...
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        ...
        summary_data = [
            ...
            {'Category': 'Target Date', 'Amount': target_date.strftime('%d-%m-%Y %H:%M')},
            ...
            {'Category': 'Total Amount Due (Principal + Interest + GST)', 'Amount': f'₹{total_amount_due:,.2f}'}
        ]
...
def main():
    ...
    st.markdown("Upload a CSV or Excel file to calculate overdue amounts with **18% per day interest**. The target date is the last date in your file at 04:43 PM IST. View results, a pie chart, and download an Excel report. *Last updated: 04:43 PM IST, August 15, 2025*")
    ...
    if uploaded_file is not None:
        ...
        st.write(f"**Target Date:** {target_date.strftime('%d-%m-%Y %H:%M')}")
